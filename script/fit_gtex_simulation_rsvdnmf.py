...
factors_out_file  = "gtex_simulation_factors_rsvdnmf.csv"
loadings_out_file  = "gtex_simulation_loadings_rsvdnmf.csv"
...
U,s,Vt = compute_rsvd(counts.T, rank=K+20, oversample=20, n_subspace=2, n_blocks=1, sparse=False, random_state=1234)
Xhat = U.dot(np.diag(s)).dot(Vt)
print("number of 0s in rsvd:" + str(Xhat[Xhat <0].shape[0]))
Xhat[Xhat <0] = 0
A,W = compute_nmf(Xhat,rank=K,init = 'nndsvd',random_state = 0,tol=1e-10, maxiter= MAX_ITER, verbose = 1)
